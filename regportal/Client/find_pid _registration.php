...
if(isset($_POST['delete_event']))
{
    ...
    if($query_run)
    {
        ...
        header("Location: find_pid _registration.php");
        exit(0);
    }
    else
    {
...
<body>
  ...
  <h1 class="bg-warning m-2" style="color:#fff">VIEW AND DELETE DATA OF <?php echo $_SESSION['f_pid'];?></h1>
  <div class="col-md-12">
    <table class="table table-bordered table-hover bg-white">
      ...
      <tbody>
        ...
        <?php
            $res1 = mysqli_query($con,"SELECT * FROM audience WHERE pid= '".$_SESSION['f_pid']."' ");
        ...
        <tr>
          ...
          <td>
            <form action="find_pid _registration.php" method="POST" class="d-inline">
              <button type="submit" name="delete_event" value="<?=$ress1['id'];?>"
                class="btn btn-danger btn-sm p-2">DELETE</button>
            </form>
          </td>
        </tr>
        <?php
  ...
  <h1 class="bg-warning m-2" style="color:#fff" >ADD Solo EVENTS To <?php echo $_SESSION['f_pid']; ?></h1>
  <section>
